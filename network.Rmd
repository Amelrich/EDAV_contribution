...
`visNetwork` est un puissant R implémentation du graphe intéractif JavaScript `vis.js` library; il utilise `tidyverse` piping: [VisNetwork Docs](https://datastorm-open.github.io/visNetwork/){target="_blank"}.

--> Le Vignette a des exemples clairement élaborés: https://cran.r-project.org/web/packages/visNetwork/vignettes/Introduction-to-visNetwork.html


Le `visNetwork` documentation ne fournit pas le même niveau d’explication que l’original, il est donc intéressant de consulter le `vis.js` documentation ainsi que:
