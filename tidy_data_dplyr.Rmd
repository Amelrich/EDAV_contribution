...
*Ce chapitre a pour origine une contribution communautaire créée par [akshatapatel](https://github.com/akshatapatel){target="_blank"}*
...
Cet exemple passe par certains travaux avec le `biopsy` dataset en utilisant `dplyr` functions pour obtenir un tidy dataset.
...
`dplyr` est utilisé pour data wrangling et data transformation dans data frames. Le "d" dans "dplyr" signifie "data frames" qui est le data type le plus utilisé pour stocker des jeux de données dans R.

## Voir les données
Commençons par charger le package afin que nous puissions obtenir les données en tant que dataframe:
```{r, include=TRUE}
...
## Qu'est ce que Tidy data?
**Qu'est-ce que cela signifie pour vos données d'être *tidy*?**

**Tidy data** a un format normalisé et constitue une manière cohérente d'organiser vos données dans R.

Voici la définition de Tidy Data donnée par Hadley Wickham:

>Un ensemble de données est en désordre ou ordonné selon la manière dont les lignes, les colonnes et les tableaux correspondent aux observations, aux variables et aux types. Dans tidy data:
>
>* Chaque variable forme une colonne.
>
>* Chaque observation forme une rangée.
>
>* Chaque observational unit forme une valeur dans la table.
>
